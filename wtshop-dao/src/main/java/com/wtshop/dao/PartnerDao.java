...
import com.wtshop.model.Partner;
...
public class PartnerDao extends BaseDao<Partner> {
	...
	public Partner findStaffList(Long memberId, Long goodId){
	 ...
	 * 查询所有有效的会员
	 */
	public List<Partner> findMemberList(Long[] memberList){
	 ...
	 *
	 */
	public List<Partner> findMemberByList(List<Long> idList){
	 ...
	 * 个人信息
	 */
	public List<Partner> findMySelfInterest(Long memberId){
	 ...
	 * 个人信息
	 */
	public List<Partner> findMySelfSkin(Long memberId){
	...
	public Page<Partner> findPages(Pageable pageable ,Integer type){
		String sql = " from partner p  LEFT JOIN member  m on  p.member_id=m.id LEFT JOIN area a ON a.id = p.area_id where 1 = 1 AND p.is_delete = 0    " ;
		if(type != null){
			sql +=" and level = " +type;
		}
		return super.findPage(sql,pageable,"select p.*,m.phone,m.nickname , a.full_name as fullName");
	}
	...
	public Partner findPartnerByMId(Long id){


		String sql = "SELECT p.*, m.phone, m.nickname, a.full_name as fullName FROM partner p LEFT JOIN member m ON p.member_id = m.id LEFT JOIN area a ON a.id = p.area_id WHERE 1 = 1 AND p.is_delete = 0 AND p.id = "+ id;
	 ...
	 */
	public Partner find(String loginPluginId, String openId) {
	 ...
	 * @return 会员，若不存在则返回null
	 */
	public Partner findByUsername(String phone) {
		if (StringUtils.isEmpty(phone)) {
			return null;
		}
		try {
	 ...
	 * @return 会员，若不存在则返回null
	 */
	public List<Partner> findListByEmail(String email) {
		if (StringUtils.isEmpty(email)) {
	 ...
	 * @return 会员分页
	 */
	public Page<Partner> findPage(Member.RankingType rankingType, Pageable pageable) {
