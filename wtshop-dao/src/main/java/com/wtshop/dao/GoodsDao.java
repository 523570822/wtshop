...
public class GoodsDao extends BaseDao<Goods> {
	...
	public Page<Goods> findSpecialGoods(Pageable pageable){
		...
		String sql = " from special_goods s LEFT JOIN goods g on s.goods_id=g.id where s.`status`=0  ORDER BY s.orders,s.create_date  ";
	...
	public List<Goods> findCartItemList(Long id) {
		String sql = "SELECT c.special_id ,c.id item_id ,p.id product_id,g.id,g.attribute_value0, g.id goodsId ,g.area_id, g.image,g.`name`,g.caption, g.caption,p.price price,p.stock-p.allocated_stock remaining,p.market_price market_price ,c.quantity FROM cart_item c left join product p on c.product_id = p.id left join goods g on p.goods_id = g.id WHERE cart_id = " + id ;
	...
	public Page<Goods> search(Boolean is_vip, List<Long> productCategoryList, Long[] brandId, List<Long> areaList, Long[] functionId, String keyword, BigDecimal startPrice, BigDecimal endPrice, Pageable pageable, Boolean modify_date, Boolean sell, Boolean review, Boolean priceUp ,Boolean priceDown){
		...
		if(keyword != null){
			...
			sqlExceptSelect += " AND (  m.`name` LIKE '%"+ keyword +"%' OR m.caption LIKE '%"+ keyword +"%' OR m.keyword LIKE '%"+ keyword +"%' or m.na1 LIKE '%"+ keyword +"%' OR m.na2 LIKE '%"+ keyword +"%' OR m.na LIKE '%"+ keyword +"%' ) ";
		}
