...
/*    public Page<GroupBuy> findPages(Pageable pageable ,boolean status){
        ...
        if(status){
              sqlExceptSelect="FROM group_buy f LEFT JOIN goods g ON f.product_id = g.id LEFT JOIN group_remind gr on f.id=gr.group_id WHERE 1 = 1 AND unix_timestamp(now()) < unix_timestamp(f.end_date) AND unix_timestamp(now()) > unix_timestamp(f.begin_date) AND f.STATUS = 0 ORDER BY price DESC";

        }else{
              sqlExceptSelect="FROM group_buy f LEFT JOIN goods g ON f.product_id = g.id LEFT JOIN group_remind gr on f.id=gr.group_id WHERE 1 = 1 AND unix_timestamp(now())<unix_timestamp( f.begin_date) AND f.STATUS = 0 ORDER BY price DESC";
        }
