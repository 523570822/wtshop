...
public class DepositLogDao extends BaseDao<DepositLog> {
	...
	public Page<DepositLog> findPage(Member member, Pageable pageable ,Integer type) {
		...
		if( type != null){
			if( 0 == type ){
				select = " SELECT * ";
				sqlExceptSelect = " FROM deposit_log WHERE member_id = " + member.getId() + " AND type in (2 , 3 , 1 ,0 ,5 ,6,7,4)";
...
//				sqlExceptSelect += " order by create_date DESC";
	...
	public Page<DepositLog> findPageBySearch(Pageable pageable, String userPhone, String nickname){
