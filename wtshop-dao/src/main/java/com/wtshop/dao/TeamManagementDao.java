...
public class TeamManagementDao extends BaseDao<TeamManagement> {
	...
	//	String sql = "";
		...
		String	sqlExceptSelect=" FROM ( SELECT count(o.id) order_num, mm.*, sum(o.price) price_num FROM ( SELECT m.id, m.avatar, m.share_code, m.create_date, m.we_chat_number, m.phone, m.nickname,m.phone_status, h.`name` housername  FROM member m LEFT JOIN houserkeeper h ON m.housekeeper_id = h.id WHERE m.on_share_code = '"+onShareCode+"' ) mm LEFT JOIN `order` o ON mm.id = o.member_id WHERE o.on_share_code = '"+onShareCode+"' AND mm.share_code IS NOT NULL GROUP BY mm.id ) dsb LEFT JOIN member dsb1 ON dsb1.link_share_code LIKE concat('%', dsb.share_code, '%')";
	...
	//	String sql = "";
		String select ="SELECT count(dsb1.id) memeber_num, dsb.* ";
		String	sqlExceptSelect=" FROM ( SELECT count(o.id) order_num, mm.*, sum(o.price) price_num FROM ( SELECT m.id, m.avatar, m.share_code, m.create_date, m.we_chat_number, m.phone, m.nickname,m.phone_status, h.`name` housername  FROM member m LEFT JOIN houserkeeper h ON m.housekeeper_id = h.id WHERE m.on_share_code = '"+onShareCode+"'  and ( m.phone LIKE concat('%', '"+str+"', '%') or m.nickname LIKE concat('%', '"+str+"', '%') or m.we_chat_number LIKE concat('%', '"+str+"', '%')) ) mm LEFT JOIN `order` o ON mm.id = o.member_id WHERE o.on_share_code = '"+onShareCode+"' AND mm.share_code IS NOT NULL GROUP BY mm.id ) dsb LEFT JOIN member dsb1 ON dsb1.link_share_code LIKE concat('%', dsb.share_code, '%')";
