...
public class TeamManagementDao extends BaseDao<TeamManagement> {
	...
	//	String sql = "";
		...
		String	sqlExceptSelect=" FROM ( SELECT count(o.id) order_num, mm.*, sum(o.price) price_num FROM ( SELECT m.id, m.avatar,m.on_share_code, m.share_code, m.create_date, m.we_chat_number, m.phone, m.nickname,m.phone_status, h.`name` housername  FROM member m LEFT JOIN houserkeeper_grade h ON m.housekeeper_id = h.id WHERE m.on_share_code = '"+onShareCode+"'  and ( m.phone LIKE concat('%', '"+str+"', '%') or m.nickname LIKE concat('%', '"+str+"', '%') or m.we_chat_number LIKE concat('%', '"+str+"', '%')) ) mm LEFT JOIN `order` o ON mm.id = o.member_id WHERE mm.on_share_code = '"+onShareCode+"'  GROUP BY mm.id ) dsb LEFT JOIN member dsb1 ON dsb1.link_share_code LIKE concat('%', dsb.share_code, '%') GROUP BY dsb.id";
