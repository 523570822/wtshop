...
public final class SystemUtils {
	...
	public static Setting getSetting() {
		...
		if (cacheElement == null) {
			...
			try {
				File wtshopXmlFile = new File(PathKit.getRootClassPath() + CommonAttributes.wtshop_XML_PATH);
				Document document = new SAXReader().read(wtshopXmlFile);

				List<org.dom4j.Element> elements = document.selectNodes("/wtshop/setting");
	...
	public static void setSetting(Setting setting) {
		...
		try {
			...
			try {
				...
				xmlWriter = new XMLWriter(new FileOutputStream(wtshopXmlFile), outputFormat);
	...
	public static TemplateConfig getTemplateConfig(String id) {
		...
		if (cacheElement == null) {
			...
			try {
				File wtshopXmlFile = new File(PathKit.getRootClassPath() + CommonAttributes.wtshop_XML_PATH);
				Document document = new SAXReader().read(wtshopXmlFile);
				org.dom4j.Element element = (org.dom4j.Element) document.selectSingleNode("/wtshop/templateConfig[@id='" + id + "']");
				if (element != null) {
	...
	public static List<TemplateConfig> getTemplateConfigs(TemplateConfig.Type type) {
		...
		if (cacheElement == null) {
			...
			try {
				File wtshopXmlFile = new File(PathKit.getRootClassPath() + CommonAttributes.wtshop_XML_PATH);
				Document document = new SAXReader().read(wtshopXmlFile);
				List<org.dom4j.Element> elements = document.selectNodes(type != null ? "/wtshop/templateConfig[@type='" + type + "']" : "/wtshop/templateConfig");
				for (org.dom4j.Element element : elements) {
	...
	public static List<LogConfig> getLogConfigs() {
		...
		if (cacheElement == null) {
			...
			try {
				File wtshopXmlFile = new File(PathKit.getRootClassPath() + CommonAttributes.wtshop_XML_PATH);
				Document document = new SAXReader().read(wtshopXmlFile);
				List<org.dom4j.Element> elements = document.selectNodes("/wtshop/logConfig");
				for (org.dom4j.Element element : elements) {
