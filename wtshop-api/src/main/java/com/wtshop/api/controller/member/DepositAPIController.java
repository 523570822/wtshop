 ...
 * 
 */
@ControllerBind(controllerKey = "/api/member/deposit")
@Before({WapInterceptor.class, ErrorInterceptor.class, TokenInterceptor.class})
public class DepositAPIController extends BaseAPIController {

	org.slf4j.Logger logger = LoggerFactory.getLogger(DepositAPIController.class);
	...
	public void log() {
		...
		Page<DepositLog> page = depositLogService.findPage(member, pageable ,type);
		DepositLogResult depositLogResult = new DepositLogResult(page);
