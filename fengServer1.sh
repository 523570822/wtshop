...
#备份语句根据实际情况修改
tar zcvf /mrmf/tomcat_shop/webapps/ROOT$(date +%Y%m%d%H).tar.gz /mrmf/tomcat_shop/webapps ;
sleep 10
echo "删除文件及缓存"
...
#打包
echo "拷贝到Tomcat下   简化后拷贝编译后的代码到指定tomcat服务器中sudo systemctl restart tomcat  "
