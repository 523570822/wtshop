...
import com.wtshop.dao.SpecialGoodsDao;
 ...
 */
public class SpecialGoodsService extends BaseService<SpecialGoods> {
    public SpecialGoodsService() {
        super(SpecialGoods.class);
    }

    private FuDaiProductDao fuDaiProductDao = Enhancer.enhance(FuDaiProductDao.class);
    private SpecialGoodsDao fuDaiDao = Enhancer.enhance(SpecialGoodsDao.class);
    ...
    public List<SpecialGoods> findLists() {
        List<SpecialGoods> dddd = fuDaiDao.findLists();
        for ( SpecialGoods fuDai:dddd) {
    ...
    public Page winRecord(Pageable pageable) {
        ...
        if (list != null && list.size() > 0) {
            for (Record record : list) {
                //获取福袋信息
                SpecialGoods fuDai = fuDaiDao.find(Long.parseLong(record.get("actOrderId")));
