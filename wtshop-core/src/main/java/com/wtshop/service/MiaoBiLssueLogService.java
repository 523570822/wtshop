...
import com.wtshop.dao.MiaoBiLssueLogDao;
 ...
 */
public class MiaoBiLssueLogService extends BaseService<MiaobiLssuelog> {
    public MiaoBiLssueLogService() {
        super( MiaobiLssuelog.class);
    }

    private FuDaiProductDao fuDaiProductDao = Enhancer.enhance(FuDaiProductDao.class);
    private MiaoBiLssueLogDao fuDaiDao = Enhancer.enhance(MiaoBiLssueLogDao.class);
    ...
    public Page winRecord(Pageable pageable) {
        ...
        if (list != null && list.size() > 0) {
            for (Record record : list) {
                //获取福袋信息
                MiaobiLssuelog fuDai = fuDaiDao.find(Long.parseLong(record.get("actOrderId")));
