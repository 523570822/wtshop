...
import com.wtshop.dao.MiaoBiLssueDao;
 ...
 */
public class MiaoBiLssueService extends BaseService<MiaobiLssue> {
    public MiaoBiLssueService() {
        super( MiaobiLssue.class);
    }

    private FuDaiProductDao fuDaiProductDao = Enhancer.enhance(FuDaiProductDao.class);
    private MiaoBiLssueDao fuDaiDao = Enhancer.enhance(MiaoBiLssueDao.class);
    ...
    public Page winRecord(Pageable pageable) {
        ...
        if (list != null && list.size() > 0) {
            for (Record record : list) {
                //获取福袋信息
                MiaobiLssue fuDai = fuDaiDao.find(Long.parseLong(record.get("actOrderId")));
